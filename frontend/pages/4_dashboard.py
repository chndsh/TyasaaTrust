...
# Scoping interactive elements and metrics layout inside a secure native layout block
with st.container(border=True):
    ...
    merchant_id = st.text_input(
        ...
        label_visibility="collapsed"  # Avoids rendering duplicate standard text input label
    )
    ...
    if st.button("Refresh scores", disabled=not is_valid, use_container_width=True):
        with st.spinner("Fetching scores..."):
            try:
                ...
                # Layout metric layout elements within columns safely inside the master container
